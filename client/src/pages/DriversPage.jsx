...
export default function DriversPage() {
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <PageHeader title="Driver Profiles" description="Manage driver compliance and performance">
            ...
            <Card><CardContent className="p-0">
                {loading ? <div className="p-4 space-y-3">{[...Array(5)].map((_, i) => <Skeleton key={i} className="h-12" />)}</div> : (
                    <Table>
                        ...
                        <TableBody>
                            {drivers.length === 0 ? <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">No drivers</TableCell></TableRow> :
                                drivers.map(d => (
                                    <TableRow key={d._id}>
